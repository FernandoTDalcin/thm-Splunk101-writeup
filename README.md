...
Sala: Splunk: The Basics | Nível: Easy | Duração: ~30 min Objetivo: entender como analistas de SOC usam o Splunk para investigar logs.
...
Settings → Add Data → Upload, selecionando o arquivo .json.
Sourcetype: _json — o Splunk reconhece automaticamente a estrutura e quebra os campos.
Criei um index dedicado (vpn_logs) pra separar esses dados de outros datasets do lab. Reparei que o campo index que já vem dentro do próprio JSON é só um dado do log — o index real é o que a gente define na hora do upload.
Explorando os dados
...
Uma quantidade de falhas muito maior que a de qualquer outro usuário.
Todas vindas do mesmo IP.
Vários timestamps idênticos — o que não parece coisa de humano digitando.
Login bem-sucedido logo depois da sequência de falhas.
Conclusão
